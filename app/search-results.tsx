...
export function SearchResults({ query, unlocked, openTravel, openForum, openActivity, openCommunity, openLostCat, openCommunityNotice, openObituary }: { query: string; unlocked: boolean; openTravel: () => void; openForum: () => void; openActivity: () => void; openCommunity: () => void; openLostCat: () => void; openCommunityNotice: () => void; openObituary: () => void }) {
  ...
  if (normalized.includes("米粒") && normalized.includes("猫")) return <div className="mt-8"><button className="search-result" onClick={openLostCat}><small className="text-[#78957e]">linchuan-pets.example · 临川寻宠互助</small><h3 className="my-3 text-xl text-[#286ab3]">寻猫启事｜米粒</h3><p className="text-xs text-[#8493a4]">灰白短毛猫，戴红色项圈。家属于8月18日发布。</p></button></div>;
