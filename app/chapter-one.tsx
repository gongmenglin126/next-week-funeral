...
import { ArrowLeft, ArrowUpRight, BedDouble, BusFront, CheckCircle2, ChevronRight, Download, FileText, LifeBuoy, MapPin, Minus, Search, Ticket, TrainFront, Waves, X } from "lucide-react";
...
export function NotesPanel({ checked, onCheck, onClose }: { checked: string[]; onCheck: (id: BookingId) => void; onClose: () => void }) {
  return <section className="desktop-panel notes-window" aria-label="记事本：雾汀行程">
    <header><div className="window-controls"><button onClick={onClose} aria-label="关闭记事本"><X /></button><button onClick={onClose} aria-label="最小化记事本"><Minus /></button></div><strong>记事本</strong><FileText /></header>
