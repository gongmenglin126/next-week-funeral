...
const CHAPTERS = [
  ...
  { number: "02", title: "潮水之前", summary: "澜序实业创办初期，以及他与陆闻川共同工作的十年。" },
...
export function FounderProfilePage() {
  return <div className="founder-profile-page">
    ...
    <main>
      ...
      <dl className="founder-achievements"><div><dt>1971年</dt><dd>出生于临川</dd></div><div><dt>1998年</dt><dd>与陆闻川共同创办澜序海工设备厂</dd></div><div><dt>2012年</dt><dd>设立临川大学海洋工程奖学金</dd></div><div><dt>2017年</dt><dd>发起安时生命关怀基金会</dd></div><div><dt>2021年</dt><dd>获评海州年度公益人物</dd></div></dl>
      <section className="founder-book-credit"><BookOpen aria-hidden="true" /><div><small>出版作品</small><h2>《把余下的时间还给别人》</h2><p>顾惟真口述自传，明川书局2021年出版。</p></div></section>
...
export function BiographyPage() {
  ...
  return <div className="biography-page">
    ...
    <main>
      <section className="biography-hero"><div className="biography-cover"><span>顾惟真<br />口述</span><strong>把余下的时间<br />还给别人</strong><small>明川书局</small></div><div><p className="biography-label">人物自传 · 2021</p><h1>把余下的时间还给别人</h1><p className="biography-subtitle">顾惟真口述，记者孟嘉整理</p><p>从临川旧城的一间设备厂，到长期投入医疗与照护公益，本书以七次访谈整理顾惟真的成长、创业和重病康复经历。</p><dl><div><dt>出版社</dt><dd>明川书局</dd></div><div><dt>出版时间</dt><dd>2021年4月</dd></div><div><dt>章节</dt><dd>全书七章</dd></div></dl></div></section>
      <section className="biography-contents"><div className="biography-section-title"><p>目录与试读</p><span>点击章节查看节选</span></div>{CHAPTERS.map((item) => <button key={item.number} onClick={() => setChapter(item.number)} aria-expanded={chapter === item.number}><span>{item.number}</span><strong>{item.title}</strong><small>{item.summary}</small></button>)}<button onClick={() => setChapter("07")} aria-expanded={chapter === "07"}><span>07</span><strong>没有回应的夜晚</strong><small>一次关于病危、祈求与幸存的访谈。</small></button></section>
